...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_workers', default=8, type=int, help="number of parallel datapool worker")
    ...
    print(predict_sb, args.score_type)
    # print(network)
    ...
    dataset = load_dataset(args.test_dirs, augment=False, scale_width=args.scale_width, split_files=args.split_files,
                           only_onsets=args.only_onsets, load_audio=args.load_audio, predict_sb=predict_sb, score_type=args.score_type)
    ...
    if args.print_piecewise:
        ...
        for piece, _ in ordering:
            ...
            if 'frame_diff' in piece_stat:
                ...
                piecewise_result[piece]["frame_error"] = cumulative_percentage
            for value in CLASS_MAPPING.values():
                if value + "_accuracy" in piece_stat:
                    print(f"\t{value} Accuracy: {piece_stat[value + '_accuracy']:.3f}")
                    piecewise_result[piece][f"{value}_Accuracy"] = piece_stat[value + '_accuracy']
                    
        p = r"D:\scorefollowersystem\cyolo_score_following\result_analysis\xml"
        np.save(f"{p}\\ablation_cyolo.npy", piecewise_result)
    print()
    for value in CLASS_MAPPING.values():
