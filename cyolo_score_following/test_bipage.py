...
from cyolo_score_following.utils.data_utils import load_bipiece_for_testing, SAMPLE_RATE, FPS, FRAME_SIZE, HOP_SIZE
...
if __name__ == '__main__':
    ...
    print(piece_name)
    org_scores, score, signal_np, systems, bars, interpol_fnc, pad, scale_factor, _ = load_bipiece_for_testing(args.test_dir, piece_name, args.scale_width)
    ...
    for page in range(p):
        ...
        while to_ <= signal_np.shape[-1]:
            ...
            # print(true_position)
            if int(true_position[-1]) >= page and int(true_position[-1])<= page + 1:
                ...
                if not args.gt_only:   
                    with torch.no_grad():
                        ...
                        x1 = np.concatenate(x1) * scale_factor 
                        x2 = np.concatenate(x2) * scale_factor 
                        y1 = np.concatenate(y1) * scale_factor - pad
                        y2 = np.concatenate(y2) * scale_factor - pad
                    ...
                    # if not args.gt_only:
                    img_pred = cv2.cvtColor(org_scores[page], cv2.COLOR_RGB2BGR)

                    if center_x > 0:
                        # print("!", actual_page,  page)
                        if actual_page == page + 1:
                            # print(center_x, w/2 * scale_factor)
                            plot_line([center_x + int(w/2 * scale_factor), center_y - pad, height], img_pred, label="GT Note", color=(0.96, 0.63, 0.25), line_thickness=2)
                        else:
                            # print(center_x, 0)
                            plot_line([center_x , center_y - pad, height], img_pred, label="GT Note", color=(0.96, 0.63, 0.25), line_thickness=2)

                    # if network.nc > 1:
                    # plot_box(xywh2xyxy(np.asarray([[system['x'], system['y'] - pad,
                    #                                     system['w'], system['h']]]))[0].astype(int).tolist(),
                    #             img_pred, color=(0.25, 0.71, 0.96), line_thickness=2, label="GT System")

                    # plot_box(xywh2xyxy(np.asarray([[bar['x'], bar['y'] - pad,
                    ...
                    # print(perf_img.shape, img_pred.shape)
        ...
        truncated_signal = signal_np[truncated_s:truncated_e]
        tag = "" if args.page is None else f"_{args.page}"

        create_video(observation_images, truncated_signal, f"{piece_name}_bipage{page}", FPS, SAMPLE_RATE, tag=tag, path=r"D:\scorefollowersystem\cyolo_score_following\result_analysis\0111ana")
